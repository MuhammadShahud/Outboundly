...
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
  if (changeInfo.status === 'complete' && tab.status === 'complete') {
    if (tab.url.includes('linkedin.com/in/')) {
      ...
      chrome.tabs.sendMessage(tabId, { message: 'loadContentScript', url: tab.url });
    }
  }
});
...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  console.log('Background Page Received Message:', message);

  if (message.type === 'showVueApp') {
    chrome.tabs.create({ url: 'index.html' });
  } else if (message.type === 'hideVueApp') {
    console.log('Background Page Received Hide Vue App Message');

    // Send a message to the content script to show the image div again
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      chrome.tabs.sendMessage(tabs[0].id, { type: 'showImageDiv' });
    });
  }
});
...
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
  if (changeInfo.status == "complete" && tab.status == "complete") {
    if (
      tab.url.includes("linkedin.com/sales/") ||
...
async function injectIfNotAsync(tabId) {
  ...
  await chrome.scripting.executeScript({
    ...
    files: ["content-linkedin.js","contentscript.js"],
  });
